...
export function buildAgentPrompt({ brandName, instructions }: AgentPromptOptions) {
  ...
  const priorityRules = [
    ...
    "- En el flujo de envio, los unicos datos realmente obligatorios son email valido y aceptacion clara del tratamiento indicado. Nombre, telefono, tipo de proyecto, interes, presupuesto y copia son opcionales o inferibles.",
    "- No bloquees el envio por no tener nombre, telefono, tipo de proyecto, interes o presupuesto. Si ya hay email y aceptacion, el sistema puede enviar el resumen.",
    "- No preguntes el mismo dato dos veces. Si el usuario ya dio email, aceptacion, nombre, presupuesto o contexto, no lo repitas.",
    "- Si el visitante dice 'envialo', 'mandalo', 'adelante' o que estas hablando demasiado, se directo: si hay email y aceptacion, confirma el envio; si falta una de esas dos cosas, pide solo lo que falte.",
